...
w_list=exp((0:.025:1)*(log(1e-2)-log(1e-6)))*1e-6;
% Number of samples
sample_num=1000;
% Timing start
tic
% Calculation result data initialization
d=zeros(length(w_list),sample_num);
% Repeat the test independently for each noise level
for level=1:length(w_list)
    for i=1:sample_num
        % Experiment once
        d(level,i)=solve_once(w_list(level));
    end
    % Timing for debug
    toc
end
% Save data for plot
save('data_noise_high','d')

%% Experiment with noise level w_max
function d=solve_once(w_max)
%% Data Initialization
% System matrix
A=[-0.192, -0.936, -0.814;
      -0.918, +0.729, -0.724;
